...
<head>
    <meta charset="utf-8">
    <title>Batch Payslip - {{ $batch->batch_id }}</title>
    <style>
        @page { margin: 0.5in; }
        body { font-family: 'Helvetica', sans-serif; color: #1f2937; margin: 0; padding: 0; background-color: #fff; }
        
        /* Batch Wrapper acts as the container */
        .payslip-wrapper { 
            width: 6.5in; 
            margin: 0 auto; 
            padding: 30px; 
            border: 1px solid #d1d5db; 
            page-break-after: always; /* Forces new page for next employee */
            position: relative;
        }

        /* Prevent blank page at the very end */
...
<div class="payslip-wrapper">
    ...
    <table class="info-grid">
        ...
        <tr>
            ...
            <td class="value">{{ date('M d', strtotime($batch->period_start)) }} - {{ date('M d, Y', strtotime($batch->period_end)) }}</td>
        </tr>
    </table>
