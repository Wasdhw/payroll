...
<div class="max-w-6xl mx-auto py-8 px-4">
    ...
    @if (session('success'))
        <div x-data="{ show: true }" x-show="show" class="mb-6 bg-teal-50 border border-teal-200 text-teal-800 px-4 py-3 rounded-lg flex justify-between items-center">
            <div>✅ {{ session('success') }}</div>
            <button @click="show = false" class="text-teal-600 hover:text-teal-900 font-bold">&times;</button>
        </div>
    @endif
        ...
        <div class="lg:col-span-1">
            <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6 sticky top-6">
                <h3 class="font-bold text-slate-700 mb-4 flex items-center gap-2">
                    ...
                    <form action="{{ route('settings.send-code') }}" method="POST">                    
                        @csrf
                    <div class="space-y-4">
                        <div>
                            <label class="block text-xs font-bold text-slate-500 uppercase mb-1">Full Name</label>
                            <input type="text" name="name" value="{{ old('name') }}" class="w-full rounded-xl border-slate-300 text-base py-3 px-4 focus:ring-teal-500 focus:border-teal-500 @error('name') border-red-500 bg-red-50 @enderror" required>
